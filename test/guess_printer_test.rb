...
class GuessPrinterTest < Minitest::Test
  ...
  def test_it_exists
    match_data = SequenceMatcher.new(GuessBuilder.new.new_guess('rrbg'), SequenceGenerator.random_sequence)
  ...
  def test_prints_no_correct_content
    ...
    skip
    match_data = SequenceMatcher.new(GuessBuilder.new.new_guess('zzzz'), SequenceGenerator.random_sequence)
    guess_printer = GuessPrinter.new(match_data)
